...
const ItemCard = ({ product }: { product: Product }) => {
    ...
    return (
        <div
            onClick={() => { router.push('/products/' + product.id); scrollTo(0, 0) }}
        ...
        >
            <div className="cursor-pointer group relative bg-gray-500/10 rounded-lg w-full h-55 flex items-center justify-center">
              <Image
                  src={product.imageUrl?.[0] ?? "/public/logo.png"}
            ...
            <div className="flex items-end justify-between w-full mt-1">
              ...
              <button onClick={(e) => {addToCart(product.id); e.stopPropagation();}} className="max-sm:hidden px-4 py-1.5 text-white border bg-black border-gray-500/20 rounded-lg text-xs hover:bg-transparent hover:text-gray-500 transition" type='button' title='add-to-cart'>
                Add to Cart
              </button>
            </div>
        </div>
