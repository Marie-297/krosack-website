  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const prodRes = await fetch(`/api/product/${id}`);
        ...
        if (proddata.success && proddata.product) {
          ...
          setHighlight(prod.highlight || []);
          setFeatures(prod.features || []);
          setBrand(prod.brand || "");
          setColor(prod.color || "");
          setStock(prod.stock?.toString() || "");
          setHighDemand(prod.highDemand || false);
          setTechnical(prod.technical || {});
          setSpecs(
            prod.technical
              ? Object.entries(prod.technical).map(([key, value]) => ({ key, value: String(value), }))
              : [{ key: "", value: "" }]
          );
          setExistingImages(prod.imageUrl || []);
